...
double MAX = 7.0; //限制距离最大值，超过这个值无论是获取还是分析都会跳过	
...
int * judge_minum_distance(double *a, double *b, cell cell_a, int first, int two, int three, save *save_a, int *chose_point,int center,double** distance) //这个输入，第一个是中心原子的坐标，第二个是你计算出来的三面共点的坐标，
...
//我们何不干脆
{
	...
	if (distancea[3] == min)
	{
		...
		cin.get();*/
		//获得了至多15个的相同距离的序号，下面判断找这些距离的最短三个,使用distance来储存距离并排序
		delete[] distancea;
		delete[] original;
